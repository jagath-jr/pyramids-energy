...
// --- Define Framer Motion animation variants for Word-by-Word ---

// Master container that triggers the stagger effect for its children (words)
const staggerContainerVariants: Variants = {
  hidden: { opacity: 0 },
  visible: {
    opacity: 1,
    transition: {
      staggerChildren: 0.08, // Time between each word animating
      delayChildren: 0.3,
    },
  },
};

// 1. Word slides in from the left
const wordSlideInLeftVariants: Variants = {
  hidden: { opacity: 0, x: -20 },
  visible: { opacity: 1, x: 0, transition: { duration: 0.5, ease: 'easeOut' } },
};

// 2. Word pops/scales in
const wordPopInVariants: Variants = {
  hidden: { opacity: 0, scale: 0.5, filter: 'blur(4px)' },
  visible: { opacity: 1, scale: 1, filter: 'blur(0px)', transition: { duration: 0.5, ease: [0.175, 0.885, 0.32, 1.275] } },
};

// 3. Word staggers up from below
const wordStaggerUpVariants: Variants = {
  hidden: { opacity: 0, y: 30 },
  visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: 'easeOut' } },
};

// 4. Word flips in 3D
const wordFlipInVariants: Variants = {
  hidden: { opacity: 0, rotateY: 90 },
  visible: { opacity: 1, rotateY: 0, transition: { duration: 0.6, ease: 'easeOut' } },
};

// --- Helper Component to split text into words ---
const AnimatedText = ({ text, variants }: { text: string; variants: Variants }) => {
  return (
    <>
      {text.split(' ').map((word, index) => (
        <motion.span
          key={index}
          variants={variants}
          className="inline-block mr-[0.25em]" // Keeps spacing between words
        >
          {word}
        </motion.span>
      ))}
              ...
              {({ isActive }) => (
                <>
                  ...
                  >
                    ...
                    {/* Slide 1: Slide in from Left (Word by Word) */}
                    {slide.animationType === 'slideInLeft' && (
                      <motion.div initial="hidden" animate={isActive ? 'visible' : 'hidden'} variants={staggerContainerVariants} className="space-y-6">
                        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight max-w-4xl">
                          <AnimatedText text={slide.title} variants={wordSlideInLeftVariants} />
                        </h1>
                        <p className="text-lg md:text-xl text-gray-200 max-w-2xl mx-auto">
                          <AnimatedText text={slide.description} variants={wordSlideInLeftVariants} />
                        </p>
                        <motion.div variants={wordSlideInLeftVariants}>
                    ...
                    {/* Slide 2: Pop In / Scale Up (Word by Word) */}
                    {slide.animationType === 'popIn' && (
                      <motion.div initial="hidden" animate={isActive ? 'visible' : 'hidden'} variants={staggerContainerVariants} className="space-y-6">
                        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight max-w-4xl">
                          <AnimatedText text={slide.title} variants={wordPopInVariants} />
                        </h1>
                        <p className="text-lg md:text-xl text-gray-200 max-w-2xl mx-auto">
                          <AnimatedText text={slide.description} variants={wordPopInVariants} />
                        </p>
                        <motion.div variants={wordPopInVariants}>
                          <Link href="/contact" className="inline-block bg-[#b92b14] hover:bg-red-800 text-white font-semibold px-8 py-3 rounded-md transition duration-300 shadow-lg">
                            Request A Quote
                          </Link>
                        </motion.div>
                    ...
                    {/* Slide 3: Staggered Fade Up (Word by Word) */}
                    {slide.animationType === 'staggerUp' && (
                      <motion.div initial="hidden" animate={isActive ? 'visible' : 'hidden'} variants={staggerContainerVariants} className="space-y-6">
                        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight max-w-4xl">
                          <AnimatedText text={slide.title} variants={wordStaggerUpVariants} />
                        </h1>
                        <p className="text-lg md:text-xl text-gray-200 max-w-2xl mx-auto">
                          <AnimatedText text={slide.description} variants={wordStaggerUpVariants} />
                        </p>
                        <motion.div variants={wordStaggerUpVariants}>
                          <Link href="/contact" className="inline-block bg-[#b92b14] hover:bg-red-800 text-white font-semibold px-8 py-3 rounded-md transition duration-300 shadow-lg">
                            Request A Quote
                          </Link>
                        </motion.div>
                    ...
                    {/* Slide 4: Flip In (Word by Word) */}
                    {slide.animationType === 'flipIn' && (
                      <motion.div initial="hidden" animate={isActive ? 'visible' : 'hidden'} variants={staggerContainerVariants} className="space-y-6" style={{ perspective: 1000 }}>
                        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight max-w-4xl">
                          <AnimatedText text={slide.title} variants={wordFlipInVariants} />
                        </h1>
                        <p className="text-lg md:text-xl text-gray-200 max-w-2xl mx-auto">
                          <AnimatedText text={slide.description} variants={wordFlipInVariants} />
                        </p>
                        <motion.div variants={wordFlipInVariants}>
                          <Link href="/contact" className="inline-block bg-[#b92b14] hover:bg-red-800 text-white font-semibold px-8 py-3 rounded-md transition duration-300 shadow-lg">
                            Request A Quote
                          </Link>
                        </motion.div>
